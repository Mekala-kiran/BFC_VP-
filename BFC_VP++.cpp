...
void printTimeDifference(const std::chrono::steady_clock::time_point& start, const std::chrono::steady_clock::time_point& end, const std::string& label) {
...
void get_graph() {
	...
	printf("[%d:] File Reading Done!\n", __LINE__);
	 auto end = std::chrono::steady_clock::now();
    	 printTimeDifference(start, end, "reading the file");
	vertices[0].clear();
	vertices[1].clear();
...
void projection(vector<vector<int>> adj)
{
	...
	for(int i=0;i<adj.size();i++)
	{
		int j = adj[i][0];
		if(i < j)
			degree_left[i] = adj[i].size();
		else
			degree_right[i] = adj[i].size();
	}	
	for(auto i:left)
	{
...
ll balanced_butterfly_counting_bucketing_wproject(vector < vector <int> > &graph) {
	ll res=0;
	ll balanced_bf_count=0;
...
void exact_algorithm_time_tracker(){
 ...
 /*for(int i=0;i<adj.size();i++)
		...
		cout<<"vertices "<<i<<":";
		for(auto j:adj[i])
		{balanced_butterfly_count_bucketing
		cout<<j<<" ";
		}
		cout<<endl;
	...
	//t_start = std::chrono::high_resolution_clock::now();		
    	t1.restart();
    	ll balanced_butterfly_count_bucketing = balanced_butterfly_counting_bucketing(adj); 
	//t_end = std::chrono::high_resolution_clock::now();	
	
	ll elapsed_time_bucketing = t1.elapsed();
	//double elapsed_time_bal_butterfly_bucketing = std::chrono::duration<double, std::milli>(t_end - t_start).count();

	printf("Total number of balanced butterflies - without projection: %lld \n Bucketing time taken: %f sec.\n",balanced_butterfly_count_bucketing , elapsed_time_bucketing/1000000.0);
	...
	//t_start = std::chrono::high_resolution_clock::now();	
	t1.restart();
    	ll balanced_butterfly_count_bucketing_wproject = balanced_butterfly_counting_bucketing_wproject(newadj); 
	//t_end = std::chrono::high_resolution_clock::now();	
	//double elapsed_time_bal_butterfly_bucketing_wproject = std::chrono::duration<double, std::milli>(t_end - t_start).count();

	printf("Total number of balanced butterflies - with projection: %lld \n Projection time: %f sec. \n Bucketing time taken: %f sec.\n",balanced_butterfly_count_bucketing_wproject , projection_time/1000000.0, t1.elapsed()/1000000.0);
	/*
	t_start = std::chrono::high_resolution_clock::now();	
